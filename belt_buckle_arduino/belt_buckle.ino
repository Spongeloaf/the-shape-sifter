...
const int number_of_inputs = 4;                                      //self explanatory, I hope - global
...
const unsigned long rollover_distance = 4294967295;									 //max int value to rollover from when ching distances
const unsigned long bin_tolerance = 1500;														 //width in dist units of the airjet to blow the part off the belt
const int number_of_bins = 16;																			 //max number of bins we can put on one belt
const unsigned long bin_airjet_time = 200;													 //time in milliseconds that the airjet stays on
const uint8_t hopper_pwm_pin = 3;																		 //PWM pin number of the hopper. Should be 2 unless shit's changeMAd somehow.
...
bool input_previous_state = true;                                    //default to true because pullup resistors invert our logic

unsigned long belt_total_distance = 0;                               //distance travelled by the belt - global
...
enum input_enum {                                                    //I don't understand how enums work. Learn this shit!  EDIT: I learned. This shit is cool.
  stick_up,
  stick_down,
  stick_left,
  stick_right
...
const int pins[number_of_inputs + 1] = {                                 //storing the pin numbers in an array is clever.
  10,           // stick_up
  11,           // stick_down
  12,           // stick_left
  13,           // stick_right
...
void setup() {
  ...
  for (int i = 0; i < number_of_inputs; i++)																									//setup our pins using a loop, makes it easier to add new pins
  {
	...
	Serial.print("[Belt Buckle v0.5.200]");																											// display program name on boot
}
      ...
      if (input_active[i] == false) {                                   //false here means input is active because of pullup resistors!
        {
          if (input_active[i] != input_previous_state)                  //checks if the state changed from our last trip through the loop
          {
            if ((millis() - lastDebounceTime) > debounceDelay)          //We use the number of milliseconds the arduino has been running for to see if it's been more than X number of millis since we last pressed a input It doesn't necessarily stop all input bouncing, but it helps. 
            {
              ...
              {
                case stick_up:
                  Serial.println("stick_up");                           //replace this with a usable action at some point
                  break;
  
                case stick_down:
                  Serial.println("stick_down");                         //replace this with a usable action at some point
                  break;
                ...
                case stick_right:
                  Serial.println("stick_right");                         //replace this with a usable action at some point
                  break;
              }
            }
          }
